...
- **Служебные страницы:** `/admin.html` и `/stats.html` защищены паролем (`gate.js`); сменить пароль — заменить SHA-256 хэш в `gate.js`.
- **Без аналитики:** сайт не использует Яндекс.Метрику, GA4, cookie и баннеры согласия — персональные данные посетителей не собираются (владелец не является оператором ПД по 152-ФЗ).
