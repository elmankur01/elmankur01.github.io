...
1. Откройте админку на живом сайте: `/4cc55b6066d79bfb2a80.html` (секретный URL, требует пароль).
...
1. Откройте админку: `/4cc55b6066d79bfb2a80.html` (секретный URL, требует пароль).
2. В блоке отправки выберите номер статьи, получателя (канал или личный чат) и нажмите кнопку.

Автопостинг работает сам: `daily_post.yml` публикует статью 4 раза в день (03:00, 09:00, 15:00 и 20:30 МСК), `daily_world_post.yml` — рубрику «Мировые новости» в 10:00 МСК, `daily_market_post.yml` — «Новости рынка» в 11:00 МСК.
...
auto-parts/
  ...
  4cc55b6066d79bfb2a80.html, admin.js — админка (секретный URL + пароль gate.js)
  4182e144696b3e5c1899.html, stats.js — панель статистики Telegram (секретный URL + пароль)
...
- **Служебные страницы:** админка `/4cc55b6066d79bfb2a80.html` и статистика `/4182e144696b3e5c1899.html` — секретные URL + пароль (`gate.js`); сменить пароль — заменить SHA-256 хэш в `gate.js`.
